...
garden_harvest %>% 
  ...
  mutate(variety_freq=fct_infreq(variety)) %>% 
  count(variety_freq)
...
Like with `fct_infreq()` directly in a plotting aesthetic:
...
garden_harvest %>% 
  filter(vegetable == "tomatoes") %>%
  ggplot(aes(x=weight,
             y=fct_reorder(variety,weight, median)))+
  geom_boxplot()
