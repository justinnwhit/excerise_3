...
garden_harvest %>% 
  mutate(lower=str_to_lower(variety)) %>% 
  summarise(unqiues = unique(lower)) %>% 
...
Trips %>% 
  mutate(time=hour(sdate)+(minute(sdate)/60),
         ...
         weekend = "weekend", ifelse((wday(sdate))=sat |sun) %>% 
...
breed_traits_total <- breed_traits %>%
  group_by(Breed) %>% 
  mutate(total_rating = sum(`Affectionate With Family`:`Mental Stimulation Needs`))) %>% 
  select(total_rating) %>%
