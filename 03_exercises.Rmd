...
garden_harvest %>% 
  ...
  summarise(t_weight=sum(weight)) %>% 
  ggplot(aes(y=t_weight,x=variety))+
  geom_col()
...
airmen %>% 
  ...
  str_pad("state",30,"both")
%>%
  ggplot(aes(y=n_names, x=rank_at_graduation, color=state))+
  geom_point(size=4)+
  
  labs(title="Tuskegee airmen with C in their name across the US by rank", y="Number of Airmen with C in their name", x = "Rank")
